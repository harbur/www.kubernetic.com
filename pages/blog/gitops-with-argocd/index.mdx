...
In this post we'll introduce ArgoCD in order to manage our application running on Kubernetes using GitOps.
...
**Infra**: We can use any ***Kubernetes*** cluster to run our components, we'll setup a [kind] cluster which
...
**MovieCatalog**: As an example we'll be deploying an app which consists of two components:
